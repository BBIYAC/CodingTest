...
# 직사각형 너비 W, 높이 H, 반지름이 H/2이고, 원1(X, Y+H/2), 원2(X+W, Y+H/2), 선수의 수 P  
w, h, x, y, p = map(int, input().split())

# 선수가 하키 링크장 안에 있는지 판별 함수
def in_hockey(px, py):
    # 원1 안에 있는지
    circle1 = (px-x)**2 + (py-(y+h/2))**2 <= (h/2)**2
    # 직사각형 안에 있는지
    square = px >= x and px <= x+w and py >= y and py <= y+h 
    # 원2 안에 있는지
    circle2 = (px-(x+w))**2 + (py-(y+h/2))**2 <= (h/2)**2
    return True if circle1 or square or circle2 else False
